...
public:
    ...
    std::vector<Node<T>*> bfs(int startNodeId) const;
    std::vector<Node<T>*> bfs(const Node<T>* startNode) const;
    std::vector<int> bfsIds(int startNodeId) const;

    std::vector<Node<T>*> dfs(int startNodeId) const;
    std::vector<Node<T>*> dfs(const Node<T>* startNode) const;
    std::vector<int> dfsIds(int startNodeId) const;

    std::vector<Node<T>*> dfsRecursive(int startNodeId) const;
    std::vector<Node<T>*> dfsRecursive(const Node<T>* startNode) const;
};
...
template<typename T>
std::vector<Node<T>*> Graph<T>::bfs(int startNodeId) const {
    std::vector<Node<T>*> result;
    ...
    q.push(const_cast<Node<T>*>(startNode));
    ...
    while (!q.empty()) {
        ...
        std::vector<Node<T>*> neighbors = getNeighbors(current);
...
template<typename T>
std::vector<Node<T>*> Graph<T>::bfs(const Node<T>* startNode) const {
    if (!startNode) {
        return std::vector<Node<T>*>();
    }
    return bfs(startNode->getId());
}

template<typename T>
std::vector<int> Graph<T>::bfsIds(int startNodeId) const {
    std::vector<Node<T>*> resultNodes = bfs(startNodeId);
    std::vector<int> ids;
...
template<typename T>
std::vector<Node<T>*> Graph<T>::dfs(int startNodeId) const {
    std::vector<Node<T>*> result;
    auto startNode = getNode(startNodeId);
    if (!startNode) return result;
    ...
    s.push(const_cast<Node<T>*>(startNode));
    visited.insert(startNodeId);
...
template<typename T>
std::vector<Node<T>*> Graph<T>::dfs(const Node<T>* startNode) const {
    if (!startNode) {
        return std::vector<Node<T>*>();
    }
    return dfs(startNode->getId());
}

template<typename T>
std::vector<int> Graph<T>::dfsIds(int startNodeId) const {
    std::vector<Node<T>*> resultNodes = dfs(startNodeId);
    std::vector<int> ids;
    for (Node<T>* node : resultNodes) {
        ids.push_back(node->getId());
    }
...
template<typename T>
std::vector<Node<T>*> Graph<T>::dfsRecursive(int startNodeId) const {
    std::vector<Node<T>*> result;
    auto startNode = getNode(startNodeId);
    if (!startNode) return result;

    std::unordered_set<int> visited;
    dfsRecursiveHelper(this, const_cast<Node<T>*>(startNode), visited, result);
    return result;
}

template<typename T>
std::vector<Node<T>*> Graph<T>::dfsRecursive(const Node<T>* startNode) const {
    if (!startNode) {
        return std::vector<Node<T>*>();
    }
    return dfsRecursive(startNode->getId());
}
